...
const ParseMessage = (message, log) => {
    const Log = log || errorEx || window.error
    ...
    const utils = {
            ...
            try {
                ...
                if (result) {
                    ...
                    if (data.indexOf('<msg>') !== -1) {
                        // xml 转换失败上报日志
                        Log('未解析出消息内容:', {message})
                    }
                    return ''
                }
            }catch (error) {
                Log('消息解析失败:', {message, error})
            }
        },
        /**
         * 链接消息解析
        ...
        parseEmoji(message) {
            try {
                ...
                if (!body.cdnurl && !body.md5) {
                    Log('Emoji表情解析为空:', {body, message})
                }
                return body
            }catch (error) {
                Log('Emoji表情解析失败:', {message, error})
                return {}
            }
        },
        ...
        parseCard(message) {
            try {
                ...
                if (result && result.msg) {
                    ...
                    Log('名片消息解析为空:', {message})
                }
                return body
            }catch (error) {
                Log('名片消息格式化出错:', {message, error})
                return {}
            }
        },
        /**
            ...
            try {
                ...
                if (result) {
                    ...
                    if (_.get(res, 'wcpayinfo.templateid') === '7a2a165d31da7fce6dd77e05c300028a') {
                        ...
                        Log('新类型红包格式化出错:', {message})
                    }
                }
                return body
            }catch (error) {
                Log('红包格式化出错:', {message, error})
                return {}
            }
        },
        /**
        ...
        parseNewYearRedPack(message) {
            try {
                ...
                if (result) {
                    ...
                    Log('拜年红包解析为空:', {message})
                }
                return body
            }catch (error) {
                Log('拜年红包解析失败:', {message, error})
                return {}
            }
        },
        /**
        ...
        parseTransfer(message) {
            try {
                ...
                if (result) {
                    ...
                    Log('转账消息解析为空:', {message})
                }
                return body
            }catch (error) {
                Log('转账消息解析失败:', {message, error})
                return {}
            }
        },
        ...
        parseInviteMember(message) {
            try {
                ...
                Log('邀请好友进群消息解析出错:', {message, error})
                return {}
            }
        },
        /**
        ...
        parseAt(lvbuffer, message) {
            if (lvbuffer) {
                if (lvbuffer.indexOf('\\\\n') !== -1) {
                    lvbuffer = lvbuffer.replace(/\\\\n/g, '')
                }
                if (lvbuffer.indexOf('\\n') !== -1) {
                    lvbuffer = lvbuffer.replace(/\\n/g, '')
                }
                const toByteBuffer = (lvbuffer) => {
                    try {
                        let byteBuffer = Base64.decode(lvbuffer)
                        if (!byteBuffer || !byteBuffer.startsWith('{') || !byteBuffer.endsWith('}')) {
                            return false
                        }
                        return byteBuffer
                    }catch (error) {
                        Log('AT解析失败:', {message, error})
                        return false
                    }
                }

                let buffer = toByteBuffer(lvbuffer)
                if (buffer) {
                    let r = /<atuserlist>(.*)<\/atuserlist>/
                    let matches = r.exec(buffer)
                    if (matches && matches.length > 1) {
                        return matches[1]
                    }else {
                        return ''
        ...
        parseCardPacket(message) {
            try {
                ...
                if (result) {
                    ...
                    Log('微信卡包解析为空:', {message})
                }
                return body
            }catch (error) {
                Log('微信卡包解析失败:', {message, error})
                return {}
            }
        },
    }
