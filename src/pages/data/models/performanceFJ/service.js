...
import {queryService} from 'data/services/performanceFJ'
...
const params = {
    ...
    by_wechat: undefined, // 1 按微信号查看
    limit: 10,
    offset: 0
}

export default {
    namespace: 'data_performance_service',
    state: {
        list: [],
        params: {
            ...params
    ...
    effects: {
        * query({payload, callback}, {select, call, put}) {
            ...
            let params = yield select(({data_performance_service}) => data_performance_service.params)
            params = {...params, ...payload.params}
            if (payload.page) {
                params.offset = params.limit * (payload.page - 1)
            }
            const data = yield call(queryService, parse({params: params}))
            if (data && data.data) {
                yield put({
                    type: 'querySuccess',
                    payload: {
