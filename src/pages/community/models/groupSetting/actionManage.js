...
import { queryActionManage, setActionManage, getActionSettingState, setActionSettingState, exportExcel } from "community/services/groupSetting/actionManage"
import moment from "moment/moment"

export default {
    namespace: 'community_groupSetting_actionManage',
    state: {
        // 命令操作
        kickMsg: '', // 显示的踢人警告語
    ...
    effects: {
        * query({payload, callback}, {select, call, put}) {
            const state = yield select(({community_groupSetting_actionManage}) => community_groupSetting_actionManage)
        ...
        * getActionSettingState({payload, callback}, {select, call, put}) { // 获取自定义/全局，use_custom
            const res = yield call(getActionSettingState, payload)
        ...
        * setActionSettingState({payload, callback}, {select, call, put}) { // 设置自定义/全局，use_custom
            const res = yield call(setActionSettingState, payload)
            if (res && res.meta?.code===200) {
                yield put({
                    type: 'setProperty',
                    payload: {
