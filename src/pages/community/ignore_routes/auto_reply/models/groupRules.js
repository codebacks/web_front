...
export default {
    namespace: 'community_groupRules',
    ...
    effects: {
        ...
        * categoryUpdate({payload, callback}, {call, put, select}) {
            const community_groupRules = yield select(({community_groupRules}) => community_groupRules)
            const formData = Object.assign({}, community_groupRules, {
                match_contents: community_groupRules.match_contents.table,
            })
        ...
        * deleteReply({payload, callback}, {call, put, select}) {
            const table = yield select(({community_groupRules}) => community_groupRules.auto_reply_reply_contents)
        ...
        * delete({payload, callback}, {call, put, select}) {
            const table = yield select(({community_groupRules}) => community_groupRules.match_contents.table)
            const index = table.findIndex((item) => item.id === payload.id)
            let newTable = table.slice()
            if(index > -1) {
                newTable.splice(index, 1)
