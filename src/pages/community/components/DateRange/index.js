...
class DateRange extends PureComponent {
    ...
    disabledStartDate = (startValue) => {
        const {endValue, maxRangeDays, maxToday, ago} = this.state
        let maxRange = false, today = false, maxDay = false
        if (maxToday) {
            today = startValue && startValue.isAfter(moment(), 'day')
        }
        if (ago) {
            maxDay = startValue && startValue.isAfter(moment().subtract(ago, 'days'), 'day')
        }
        if (!startValue || !endValue) {
            return maxToday ? today : ago ? maxDay : false
        }
        if (maxRangeDays) {
            maxRange = endValue.diff(startValue, 'days') > maxRangeDays
        }
        return startValue.isAfter(endValue, 'day') || maxRange || today || maxDay
    };

    disabledEndDate = (endValue) => {
        const {startValue, maxRangeDays, maxToday, ago} = this.state
        let maxRange = false, today = false, maxDay = false
        if (maxToday) {
            today = endValue && endValue.isAfter(moment(), 'day')
        }
        if (ago) {
            maxDay = endValue && endValue.isAfter(moment().subtract(ago, 'days'), 'day')
        }
        ...
        return endValue.isBefore(startValue, 'day') || maxRange || today || maxDay
    };
