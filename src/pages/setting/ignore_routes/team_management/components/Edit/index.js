...
const transformArr = ['nickname', 'role', 'remark', 'type']
...
@Form.create({
    mapPropsToFields(props) {
        return objToAntdForm(_.get(props, 'setting_teamManagement.userForm', {}), transformArr)
    },
    onFieldsChange(props, field, fields) {
        props.dispatch({
            type: 'setting_teamManagement/setUserForm',
            payload: field,
        })
    },
})
class Index extends Component {
    ...
    selectDepartment = () => {
        this.props.dispatch({
            type: 'setting_teamManagement/selectTree',
            payload: {},
        })
        this.setState({
            visible: true,
    ...
    handleSubmit = (e) => {
        ...
        this.props.form.validateFields((err, values) => {
            if(!err) {
                const {userForm} = this.props.setting_teamManagement
                if(_.get(userForm, 'departments.length', 0) < 1) {
                    message.error('部门不能为空!')
                    return
                }
                this.props.dispatch({
                    type: 'setting_teamManagement/updateUser',
                    payload: {},
                    callback: () => {
                        this.props.goPage()
                        this.onCancel()
    ...
    setTreeTitleName = (name) => {
        const {searchValue} = this.props.setting_teamManagement
    ...
    renderTreeTitle = (item) => {
        ...
        const select = !!_.get(this.props, 'setting_teamManagement.userForm.departments', []).find(department => {
            return department.id === item.id
        })
    ...
    onLeaderChange = (item) => {
        ...
        this.props.dispatch({
            type: 'setting_teamManagement/updateSelectTree',
            payload: {},
        })
    }
    ...
    renderSelectedDepartments = () => {
        const departments = _.get(this.props, 'setting_teamManagement.userForm.departments', [])
    ...
    validateRole = (rule, value, callback) => {
        ...
        if(getFieldValue('type') !== '1' && value === 0) {
            callback('必填')
        }else {
            callback()
        }
    ...
    renderType = ({userForm, getFieldValue, formItemLayout, getFieldDecorator, typeMap}) => {
    ...
    onExpand = (expandedKeys) => {
        this.props.dispatch({
            type: 'setting_teamManagement/setProperty',
            payload: {selectTreeExpandedKeys: expandedKeys},
        })
    }
    ...
    onChange = (e) => {
        ...
        this.props.dispatch({
            type: 'setting_teamManagement/searchTreeChange',
            payload: value,
        })
    }
    ...
    render() {
        ...
        const {roles, userForm, selectTree, selectTreeExpandedKeys, searchValue} = this.props.setting_teamManagement
        ...
        return (
            ...
            >
                <Form layout={'horizontal'}>
                    <Row className={styles.accountRow}>
                        ...
                        <Col span={14}>
                            ...
                            <span className={styles.enabled}>
                                {Number(userForm.enabled) === 1 ? '启用' : '禁用'}
                            </span>
                        </Col>
                    </Row>
                    {/*<Row className={styles.accountRow}>*/}
                    ...
                    {
                        this.renderType({userForm, getFieldValue, formItemLayout, getFieldDecorator, typeMap})
                    }
                    {
