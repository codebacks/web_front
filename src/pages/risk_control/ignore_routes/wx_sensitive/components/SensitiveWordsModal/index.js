    ...
    handleOk = (e) => {
        ...
        this.props.form.validateFields((err, values) => {
            if (!err) {
                ...
                values.behavior_type = Number(values.behavior_type)
                if (isCreate) {
                    values.sensitive_words = [...new Set(this.getNamesArr(values.sensitive_words))]
                    this.props.dispatch({
                        type: 'risk_control_wxSensitive/createWxSensitiveWord',
                        payload: {
                            body: values,
                        },
                        callback: () => {
                    ...
                    this.props.dispatch({
                        type: 'risk_control_wxSensitive/changeWxSensitiveWord',
                        payload: {
                            id: record.id,
                            body: values,
                        },
            ...
            <Form layout={'horizontal'} {...formItemLayout}>
                {
                    isCreate ? (
                        ...
                        <FormItem label="敏感词" required={true}>
                            {getFieldDecorator('sensitive_word', {
                                initialValue: record.sensitive_word,
                ...
                <FormItem label="类型" required={true}>
                    {getFieldDecorator('behavior_type', {
                        initialValue: record.behavior_type,
                        rules: [
                            {
                                required: true,
                                message: '必填',
                        ...
                        <Radio.Group>
                            <Radio value={0}>{'警告'}</Radio>
                            <Radio value={1}>{'警告并拦截'}</Radio>
                        </Radio.Group>,
                    )}
                </FormItem>
            </Form>
