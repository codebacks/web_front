...
export default {
    ...
    effects: {
        ...
        * autoReplyUpdate({payload, callback}, {call, put}) {
            const {data} = yield call(autoReplyUpdate, {status: Number(payload)})
            if(data) {
                yield put({
                    type: 'setProperty',
                    payload: {
        ...
        * autoReply({payload}, {call, put, select}) {
            let params = yield select(({wx_autoReply}) => wx_autoReply.params)
