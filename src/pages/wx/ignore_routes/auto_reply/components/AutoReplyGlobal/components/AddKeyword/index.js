...
import AddKeywordReply from 'wx/components/AddKeywordReply'
...
export default class AddKeyword extends React.Component {
    ...
    componentWillUnmount() {
        this.props.dispatch({
            type: 'wx_autoReply_global/setProperty',
        ...
        this.props.form.validateFields({force: true}, (err, values)=> {
            if(!err) {
                ...
                && values.content === record?.content && values.match_type === record?.match_type) { // 未修改内容的情况
                    ...
                    this.props.dispatch({
                        type: `wx_autoReply_global/${type === 'edit' ? 'editKeyword': 'addKeyword'}`,
                        payload: {
                            template_id: this.props.match?.params?.id,
                            keyword_id: record?.id, // 编辑的时候需要带record.id
                            body: {...values},
                        },
                        callback: () => {
                            message.success(`${type === 'edit' ? '修改成功！': '创建成功！'}`)
                            this.props.onModalCancel()
                            onOk && onOk()
                        }
                    })
    ...
    render() {
        ...
        const { replyObj } = this.props.wx_autoReply_global
        ...
        /*const list = [{
            auto_reply_category_id: 41,
            common_msg_content: {
                source_type: 2,
                type: 1,
                values: {content: "12121"},
            },
            common_msg_content_source_type: 2,
            common_msg_content_type: 1,
            // common_msg_content_values: "{"content": "12121"}",
            content: "12121",
            id: 0,
            msg_type: 1,
            ord: 1,
