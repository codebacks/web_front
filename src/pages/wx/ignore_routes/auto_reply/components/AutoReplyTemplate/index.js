...
@connect(({wx_autoReplyTemplate, loading}) => ({
    wx_autoReplyTemplate,
    queryLoading: loading.effects['wx_autoReplyTemplate/query'],
    addLoading: loading.effects['wx_autoReplyTemplate/add'],
    editLoading: loading.effects['wx_autoReplyTemplate/edit'],
    removeLoading: loading.effects['wx_autoReplyTemplate/remove'],
}))
export default class Index extends PureComponent {
    ...
    goPage = page => {
        if(typeof page === 'undefined') {
            page = this.props.wx_autoReplyTemplate.current
        }
        this.props.dispatch({
            type: 'wx_autoReplyTemplate/query',
            payload: {page: page},
        })
    }
    ...
    remove = (record) => {
        new  Promise((resolve, reject) => {
            this.props.dispatch({
                type: 'wx_autoReplyTemplate/removeCheck',
                payload: {
                ...
                Modal.confirm({
                    ...
                    onOk: () => {
                        return new Promise((resolve) => {
                            this.props.dispatch({
                                type: 'wx_autoReplyTemplate/remove',
                                payload: {
                                    template_id: record?.id,
                                },
                                callback: () => {
                                    ...
                                    this.goPage(this.props.wx_autoReplyTemplate.current ||1)
                                }
                            })
                        })
                    },
    ...
    goDetail = (record) => {
        router.push(`/wx/auto_reply/templateDetail/${record.id}`)
    }
    ...
    onUpdateTemplateModalOk = (type, inputValue, record) => {
        ...
        if(type === 'add') { // add
            ...
            dispatch({
                type: 'wx_autoReplyTemplate/edit',
                payload: {
                    template_id: record?.id,
                    body: {
                        title: inputValue,
    ...
    render() {
        const { queryLoading, addLoading, editLoading, wx_autoReplyTemplate,  } = this.props
        const { list, params: {limit}, current, total } = wx_autoReplyTemplate
