...
@connect(({wx_autoReply_templateDetail, loading}) => ({
    wx_autoReply_templateDetail,
    queryLoading: loading.effects['wx_autoReply_templateDetail/query'],
}))
export default class Index extends PureComponent {
    ...
    getData = () => {
        this.props.dispatch({
            type: 'wx_autoReply_templateDetail/query',
            payload: {
    ...
    move = (record, preIndex, nextIndex) => {
        const { list } = this.props.wx_autoReply_templateDetail
        let ord = 0
        if(nextIndex === 0) {
        ...
        console.log('ord:', ord)
        this.props.dispatch({
            type: 'wx_autoReply_templateDetail/move',
            payload: {
                template_id: this.props.match?.params?.id,
    ...
    removeKeyword = (record) => {
        this.props.dispatch({
            type: 'wx_autoReply_templateDetail/removeKeyword',
            payload: {
                template_id: this.props.match?.params?.id,
                keyword_id: record.id,
            },
    ...
    render() {
        const { queryLoading, wx_autoReply_templateDetail, } = this.props
        const { list, } = wx_autoReply_templateDetail
                            ...
                            renderBtn={(setTrue) => {
                                return (
                                    <span
                                        ...
                                        onClick={() => {
                                            this.props.dispatch({
                                                type: 'wx_autoReply_templateDetail/getReplyContents',
                                                payload: {id: text},
                                                callback: (data) => {
                                                    this.setState({
