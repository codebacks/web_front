...
import api from 'wx/common/api/autoReplyModal'
import {format} from 'utils'
import qs from 'qs'

export async function getSetting(payload, params) {
    return request(`${format(api.getSetting.url, {uin: payload.uin})}?${qs.stringify(params)}`)
}
...
export async function setSetting(payload) {
    return request(format(api.setSetting.url, {uin: payload.uin}), {
