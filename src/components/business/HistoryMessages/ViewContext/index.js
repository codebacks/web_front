...
export default class ViewContext extends Component {
    ...
    getFromUin = (activeSession) => {
        return _.get(activeSession, 'from.uin') || _.get(activeSession, 'from_uin') || _.get(activeSession, 'friend.from.uin')
    }

    getToUsername = (activeSession) => {
        return _.get(activeSession, 'target.username') || _.get(activeSession, 'wechat.username') || _.get(activeSession, 'friend.target.username')
    }

    fetchData = async (type) => {
        const {list: prevList} = this.props.messages
        let params = {...this.state.params}
        ...
        if (type === 'init') {
            ...
            if (responseData && responseData.length) {
                ...
                if (pagination.rows_found <= pagination.limit) {
                    ...
                    console.log('init load once more')
                    this.setState({
                        disableBottomFetch: true
                    })
                    ...
                    // console.log('init load once more', params)
                    ...
                    if (index / params.limit === 0) {
                        ...
                        params.offset = offset - limit
                        let res2 = await this.loadMessages('top', params)
                        // console.log('index / params.limit', params)
                        this.setProperty('top', params, res2.pagination)
                        data = res2.data.concat(data)
                        // 无更多内容
                        if (params.offset === 0) {
    ...
    loadMessages = async (type, params) => {
        try {
            ...
            const res = await request(`${helper.format(API.rawMessages.url, {
                uin: params.from_uin,
