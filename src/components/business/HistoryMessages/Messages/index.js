...
@connect(({messages, search_messages}) => ({
    messages,
    search_messages,
}))
export default class Messages extends PureComponent {
    ...
    componentDidMount() {
        ...
        if (keyword) {
            let params = {...this.props.search_messages.params}
            params.key = keyword
            this.props.dispatch({
                type: 'search_messages/setProperty',
                payload: {
                    params: params,
                    searchParams: {...params}
                }
    ...
    reset = () => {
        ...
        this.props.dispatch({
            type: 'search_messages/resetParams'
        })
        this.props.dispatch({
            type: 'search_messages/setProperty',
            payload: {
    ...
    fetchData = async (type) => {
        const keyword = this.props.search_messages.params.key
    ...
    fetchNormalData = async (type) => {
        ...
        const {params: messagesParams, list: prevList, total} = this.props.messages
    ...
    fetchKeywordData = async (type) => {
        // console.log('fetchKeywordData')
        const {params: messagesParams} = this.props.messages
        let {params: searchMessagesParams, total} = this.props.search_messages
        const namespace = 'search_messages'
    ...
    pushImages = (type, data, isInit) => {
        let {images: prevImages} = this.props.messages
        if (isInit) {
            prevImages = []
        }
        let parseImages = []
    ...
    loadMessages = async (params) => {
        try {
            ...
            return await request(`${helper.format(API.rawMessages.url, {
                uin: params.from_uin,
    ...
    loadKeywordMessages = async (params) => {
        try {
            ...
            const res = await request(`${helper.format(API.searchMessages.url, {
                uin: params.uin,
            })}?${qs.stringify(query)}`)
            this.props.dispatch({
                type: 'search_messages/setProperty',
    ...
    handleInputChange = (e) => {
        ...
        new Promise((resolve, reject) => {
            this.props.dispatch({
                type: 'search_messages/setParams',
                payload: {params: params},
            })
            resolve()
        }).then(() => {
            if (!value && this.props.search_messages.searchParams.key) {
                this.handleSearch()
            }
        })
    }
    ...
    handleSearch = () => {
        ...
        const type = this.props.messages.params.type || ''
        const {params: {key: keyword}, searchParams} = this.props.search_messages
    ...
    resetSearchParams = () => {
        this.props.dispatch({
            type: 'search_messages/resetSearchParams',
        })
    }
    ...
    getView = (type) => {
        const {searchParams: {key: keyword}, total} = this.props.search_messages
        const {images, audio} = this.props.messages
    ...
    render() {
        const {params} = this.props.messages
        const {params: keywordParams, searchParams: keywordSearchParams} = this.props.search_messages
        ...
        return (
            <div className={styles.history}>
                ...
                {
                    keywordSearchParams.key && visible ? <ViewContext {...this.props}
