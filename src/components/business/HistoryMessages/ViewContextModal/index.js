...
export default class Index extends React.Component {
    ...
    componentDidMount() {
        ...
        this.props.dispatch({
            type: 'messages/wxTalkersDetail',
            payload: {
                uin: fromUin,
                username: toUsername,
            },
            callback: (data = {})=>{
                const newActiveSession = {...this.state.activeSession}
                const friend = data.friend || {}
                newActiveSession.from = {
                    ...newActiveSession.from,
                    ...friend.from
                }
                newActiveSession.target = {
                    ...newActiveSession.target,
                    ...friend.target
                }
                this.setState({
    ...
    getInitParams = () => {
        const {activeRecord, fromUin, toUsername} = this.props
        let params = {...this.state.params}
        params.location_create_time = util.getCreateTime(activeRecord)
        params.from_uin = fromUin
        params.to_username = toUsername
        return params
    }
    ...
    render() {
        ...
        const {disableTopFetch, disableBottomFetch, activeSession} = this.state
