...
export function setHOCDisplayName(component, name = 'leo') {
...
class SaveExecuteOnce {
    ...
    fire(eventName, fn) {
        this.events.indexOf(eventName) === -1 && fn()
        this.save(eventName)
    }
    off(eventName) {
        let index = this.events.indexOf(eventName)
...
const __SAVE__EXECUTE__ONCE__ = new SaveExecuteOnce()

/**
* 在组件事件执行只一次
* @param {String} eventName 
 * @param {Function} fn 
 * @param {Component} context 执行的组件的 this
 */
export function $once(eventName, fn, context) {
    const execute = __SAVE__EXECUTE__ONCE__
    if (typeof fn === 'function') {
        if (context && !context.__REACT__ONCE__) {
            context.__REACT__ONCE__ = true
            execute.clear()
        }
        execute.fire(eventName, fn)
    }
}